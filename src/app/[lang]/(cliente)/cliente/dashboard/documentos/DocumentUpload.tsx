...
export default function DocumentUpload({
  ...
  cases: Case[]
  labels: { case_label: string; select_file: string; uploading: string }
}) {
  ...
  return (
    <div className="flex gap-3 items-end flex-wrap">
      <div className="flex-1 min-w-[200px]">
        <p className="text-sm text-muted-foreground mb-1">{labels.case_label}</p>
      ...
      <Button onClick={() => inputRef.current?.click()} disabled={uploading || !selectedCase}>
        ...
        {uploading ? labels.uploading : labels.select_file}
      </Button>
    </div>
  )
}
