...
function LoginForm() {
  ...
  async function onSubmit(data: FormData) {
    ...
    if (redirect) {
      ...
      router.push(dashboards[profile?.role ?? ''] ?? `/${lang}`)
    }
  }
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="mt-4 text-center space-y-2">
          <Link href={`/${lang}/recuperar-senha`} className="text-sm text-primary hover:underline block">
            Esqueceu sua senha?
          </Link>
          <p className="text-sm text-muted-foreground">
            Não tem conta?{' '}
            <Link href={`/${lang}/cadastro`} className="text-primary hover:underline">
              Cadastre-se
            </Link>
          </p>
        </div>
